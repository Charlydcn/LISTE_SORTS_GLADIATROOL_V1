-- Réinitialisation atomique d'un groupe de propriétés vers leur baseline JSON.
-- Chaque valeur réellement différente est historisée, puis l'override est supprimé.
...
begin
  ...
  -- Ordre stable pour éviter les interblocages entre deux réinitialisations concurrentes.
  for v_target in
    select item.value
  ...
  loop
    ...
    if v_entity_type not in ('spell', 'class_stat')
...
comment on function public.reset_overrides(jsonb) is
  'Historise les retours à la baseline puis supprime les overrides dans une transaction unique.';
