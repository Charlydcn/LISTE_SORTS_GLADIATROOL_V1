-- Ajoute la description aux propriétés éditables des sorts.
...
alter table public.entity_overrides
  add constraint entity_overrides_allowed_field check (
    (entity_type = 'spell' and field_key in (
      'nom', 'description', 'pa', 'po', 'porteeModifiable', 'lancerEnLigne', 'ligneDeVue',
...
alter function public.apply_override(text, text, text, jsonb, jsonb)
  rename to apply_override_without_description;

revoke all on function public.apply_override_without_description(text, text, text, jsonb, jsonb)
...
begin
  if p_field_key <> 'description' then
    return query select * from public.apply_override_without_description(
  ...
  if p_entity_type <> 'spell' or nullif(p_entity_key, '') is null
    or jsonb_typeof(v_new_value) <> 'string' then
    raise exception 'Invalid spell description' using errcode = '22023';
  end if;
  ...
  where entity_type = p_entity_type and entity_key = p_entity_key and field_key = p_field_key
  for update;
  v_exists := found;
  ...
  if v_exists then
    ...
    set value = v_new_value, previous_value = v_old_value, updated_at = v_saved_at,
        updated_by = v_user_id, updated_by_label = v_author_label
