...
function SpellReference({ spell }: { spell: Spell | undefined }) {
  ...
  return <div className="source-spell"><div className="source-spell-head"><SpellIcon spell={spell} /><div><strong>{spell.nom}</strong><span>Sort #{spell.id}</span></div></div><div className="source-effects"><div><strong>Effets normaux</strong>{normal.map((effect, index) => <p key={index}>{effect.texte}</p>)}</div><div><strong>Effets critiques</strong>{critical.length ? critical.map((effect, index) => <p key={index}>{effect.texte}</p>) : <p>Aucun.</p>}</div></div></div>;
}
...
function TonicCreator({ kind, className, onCreated, onCancel }: { kind: "tonique" | "mutation"; className?: string; onCreated: (tonic: Tonic) => void; onCancel: () => void }) {
  const [title, setTitle] = useState(""); const [effects, setEffects] = useState(""); const [category, setCategory] = useState<TonicCategory>(kind === "mutation" ? "mutation" : "palier1"); const [spell, setSpell] = useState<Spell | null>(null); const [busy, setBusy] = useState(false);
  ...
  return <form className="spell-card tonic-creator" onSubmit={submit}><h2>{kind === "mutation" ? "Nouvelle mutation" : "Nouveau tonique"}</h2><label>Titre<input required value={title} onChange={(event) => setTitle(event.target.value)} /></label>{kind === "tonique" ? <label>Catégorie<select value={category} onChange={(event) => setCategory(event.target.value as TonicCategory)}><option value="palier1">Palier 1</option><option value="palier2">Palier 2</option><option value="rarus">Rarus</option></select></label> : <><label>Sort associé</label><SpellPicker className={className!} selected={spell?.id ?? null} onSelect={setSpell} />{spell ? <SpellReference spell={spell} /> : <p className="empty-state">Choisis un sort pour afficher ses effets actuels.</p>}</>}<label>Effets<textarea rows={7} value={effects} onChange={(event) => setEffects(event.target.value)} placeholder="Un effet par ligne" /></label><div className="editor-actions"><button className="primary-button" type="submit" disabled={busy || (kind === "mutation" && !spell)}>Créer</button><button className="secondary-button" type="button" onClick={onCancel}>Annuler</button></div></form>;
}
...
export function MutationClassPage() {
  const { classe } = useParams(); const className = decodeURIComponent(classe ?? ""); const mutations = useTonicStore((s) => s.tonics.filter((tonic) => tonic.kind === "mutation" && tonic.className === className)); const isAdmin = useSessionStore((s) => s.mode) === "admin"; const [selected, setSelected] = useState<number | null>(mutations[0]?.id ?? null); const [creating, setCreating] = useState(false); const mutation = mutations.find((item) => item.id === selected);
  ...
  return <><div className="page-heading"><div><a className="back-link" href="#/mutations">← Toutes les classes</a><h1>{className} — Mutations</h1><p>{mutations.length} mutations actuelles.</p></div>{isAdmin ? <button className="new-spell-button" type="button" onClick={() => { setCreating(true); setSelected(null); }}>Nouvelle mutation</button> : null}</div><div className="catalog-layout"><div><div className="mutation-list">{mutations.map((item) => <button className={selected === item.id ? "selected" : ""} type="button" key={item.id} onClick={() => { setCreating(false); setSelected(item.id); }}><span>{item.title}</span><small>Sort #{item.spellId ?? "—"}</small></button>)}</div>{isAdmin ? <DeletedNativeList kind="mutation" className={className} onRestore={(item) => setSelected(item.id)} /> : null}</div><aside>{creating ? <TonicCreator kind="mutation" className={className} onCreated={(item) => { setCreating(false); setSelected(item.id); }} onCancel={() => setCreating(false)} /> : mutation ? <TonicCard tonic={mutation} onDeleted={() => setSelected(null)} /> : <div className="detail-placeholder">Sélectionne une mutation.</div>}</aside></div></>;
}
